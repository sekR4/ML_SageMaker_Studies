...
def test_numerical_df(numerical_dataframe):
    ...
    assert isinstance(transformed_df, pd.DataFrame), "Returned type is {}.".format(
        type(transformed_df)
    )
    ...
    assert (
        transformed_df.loc[0, "Category"] == 1
    ), "`heavy` plagiarism mapping test, failed."
    assert (
        transformed_df.loc[2, "Category"] == 0
    ), "`non` plagiarism mapping test, failed."
    assert (
        transformed_df.loc[30, "Category"] == 3
    ), "`cut` plagiarism mapping test, failed."
    assert (
        transformed_df.loc[5, "Category"] == 2
    ), "`light` plagiarism mapping test, failed."
    assert (
        transformed_df.loc[37, "Category"] == -1
    ), "original file mapping test, failed; should have a Category = -1."
    assert (
        transformed_df.loc[41, "Category"] == -1
    ), "original file mapping test, failed; should have a Category = -1."
...
def test_containment(complete_df, containment_fn):
    ...
    assert test_val <= 1.0, (
        "It appears that the value is not normalized; expected a value <=1, got: "
        + str(test_val)
    )
    ...
    print(results_1gram, ngram_1, sep="\n")
    assert all(
        np.isclose(results_1gram, ngram_1, rtol=1e-04)
    ), "n=1 calculations are incorrect. Double check the intersection calculation."
    # check correct results
    assert all(
        np.isclose(results_3gram, ngram_3, rtol=1e-04)
...
def test_lcs(df, lcs_word):
    ...
    # check correct results
    assert all(
        np.isclose(results, lcs_vals, rtol=1e-05)
    ), "LCS calculations are incorrect."
...
def test_data_split(train_x, train_y, test_x, test_y):
    ...
    assert len(train_x) + len(test_x) == 95, (
        "Unexpected amount of train + test data. Expecting 95 answer text files, got "
        + str(len(train_x) + len(test_x))
    )
    assert (
        len(test_x) > 1
